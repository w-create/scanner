...
BOOL optionSetting() {
	...
	ret_value = SetResolution(ICAP_XRESOLUTION, 1);
	cout << "ICAP_PIXELTYPE "; cout << ICAP_XRESOLUTION << endl;

	ret_value = SetResolution(ICAP_YRESOLUTION, 300);
	cout << "ICAP_YRESOLUTION "; cout << ret_value << endl;


	ret_value = SetCapability(ICAP_JPEGPIXELTYPE, TRUE,TRUE);
	cout << "ICAP_JPEGPIXELTYPE "; cout << ret_value << endl;

	ret_value = SetCapability(CAP_CAPTION, 6, TRUE);
	cout << "CAP_PAPERDETECTABLE "; cout << ret_value << endl;
	...
	CallTwainProc(&AppID, &m_Source, DG_CONTROL, DAT_CUSTOMDSDATA, MSG_GET,&twCap);
//	pTW_ONEVALUE pVal;
		
	pVal = (pTW_ONEVALUE)GlobalLock(twCap.hContainer);
	
	cout << "twCap result "; cout << pVal->ItemType << endl;
	cout << "twCap result "; cout << pVal->Item << endl;
...
BOOL GetImage(TW_IMAGEINFO& info)
{
	...
	case TWRC_XFERDONE: {
		...
		//void* lp = ::GlobalLock((HGLOBAL)hBitmap);
		// 复制
		//memcpy(lpCopy, lp, (info.ImageWidth*3*info.ImageLength));
		// 解除锁定
		//::GlobalUnlock(hCopy);
		...
		int widthBitTemp = widthBit;
		while (widthBitTemp % 4) widthBitTemp++;
		pBits = lpVoid + sizeof(BITMAPINFOHEADER);
		m_pBits = (unsigned char *)malloc(height*widthBitTemp);
		memcpy(m_pBits, pBits, height*widthBitTemp);
		...
		int x = 0;
		int totalPex = height * widthBit;
		oo = 0;

		while ((x + 3) <= totalPex) {
			unsigned char temp = *convert;
			*convert = *(convert + 2);
			*(convert + 2) = temp;
			oo = oo + 3;
			x = x + 3;
